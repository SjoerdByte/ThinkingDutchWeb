...
if (!(isset($response['success']) ? $response['success'] : false)) {
    ...
    header('Location: ../nl/carsys.php?error=reCAPTCHA verification failed');
    exit;
}
...
if (strpos($json, "success") !== false) {
    $body = "<div>
        ...
        <b>Interesse in:</b> " . (is_array($interest) ? implode(", ", $interest) : htmlspecialchars($interest)) . "<br>
    ...
    if (mail('carsysform@thinkingdutch.com', 'Carsys contact formulier - ThinkingDutch.com', $body, $headers)) {
        ...
        header('Location: ../nl/carsys.php?error=Failed to send email.');
        exit();
    }
} else {
    error_log('WhatsApp message sending failed.');
    header('Location: ../nl/carsys.php?error=WhatsApp message sending failed.');
    exit();
}
